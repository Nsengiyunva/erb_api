...
//     // ERBPaid (erb_paid_list) has no picture field of its own — profile
//     // pictures live on OldUser, joined here by email. email_address on
//     // ERBPaid can hold multiple addresses separated by ';' (same format the
//     // frontend already splits on), so we match against the first one.
//     const plainRecords = records.map(r => r.toJSON() as any)
//     const primaryEmails = Array.from(new Set(
//       plainRecords
//         .map(r => r.email_address?.split(';')[0]?.trim().toLowerCase())
//         .filter(Boolean)
//     ))

//     if (primaryEmails.length > 0) {
//       const matchedUsers = await OldUser.findAll({
//         where: { email: { [Op.in]: primaryEmails } },
//         attributes: ['email', 'profile_picture', 'user_picture'],
//       })

//       // Map lowercased email -> resolved picture URL, via the same
//       // /old/users/uploads/:filename route serveProfilePicture exposes,
//       // which handles both the uploads/users/ and legacy bare-filename cases.
//       const pictureByEmail = new Map<string, string>()
//       matchedUsers.forEach((u: any) => {
//         const pic = u.profile_picture || u.user_picture
//         if (!pic || !u.email) return
//         const filename = path.basename(pic)
//         pictureByEmail.set(
//           u.email.toLowerCase(),
//           `https://data.erb.go.ug/old/users/uploads/${filename}`
//         )
//       })

//       plainRecords.forEach(r => {
//         const primaryEmail = r.email_address?.split(';')[0]?.trim().toLowerCase()
//         r.profile_picture_url = primaryEmail ? (pictureByEmail.get(primaryEmail) || null) : null
//       })
//     } else {
//       plainRecords.forEach(r => { r.profile_picture_url = null })
//     }
...
export const getAllPaidRecords = async (req: Request, res: Response) => {
  try {
    ...
    // ── attach profile pictures ─────────────────────────────────────────────
    // ERBPaid (erb_paid_list) has no picture field of its own — profile
    // pictures live on OldUser. Join by registration number: ERBPaid.reg_no
    // (e.g. "TR 265") against OldUser.licence_no — email matching was tried
    // first but reg_no is the reliable key (confirmed via manual DB check).
    const plainRecords = records.map(r => r.toJSON() as any)
    ...
    if (regNos.length > 0) {
      const matchedUsers = await OldUser.findAll({
        where: { licence_no: { [Op.in]: regNos } },
        attributes: ['licence_no', 'profile_picture', 'user_picture'],
      })

      // Map licence_no -> resolved picture URL, via the same
      // /old/users/uploads/:filename route serveProfilePicture exposes,
      // which handles both the uploads/users/ and legacy bare-filename cases.
      const pictureByRegNo = new Map<string, string>()
      matchedUsers.forEach((u: any) => {
        const pic = u.profile_picture || u.user_picture
        if (!pic || !u.licence_no) return
        const filename = path.basename(pic)
        pictureByRegNo.set(
          u.licence_no.trim(),
          `https://data.erb.go.ug/old/users/uploads/${filename}`
      ...
      plainRecords.forEach(r => {
        ...
        r.profile_picture_url = regNo ? (pictureByRegNo.get(regNo) || null) : null
      })
    } else {
      plainRecords.forEach(r => { r.profile_picture_url = null })
    }

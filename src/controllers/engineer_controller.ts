...
export const getAllPaidRecords = async (req: Request, res: Response) => {
  try {
    ...
    // pictures live on OldUser. Neither key alone is reliable enough on its
    // own (some OldUser rows only have a matching licence_no, others only a
    // matching email — confirmed via manual DB checks), so match on both
    // and prefer the reg_no/licence_no hit when both exist.
    const plainRecords = records.map(r => r.toJSON() as any)
    const regNos = Array.from(new Set(
      plainRecords.map(r => r.reg_no?.trim()).filter(Boolean)
    ))
    ...
    if (regNos.length > 0 || primaryEmails.length > 0) {
      const orConditions: any[] = []
      if (regNos.length > 0)       orConditions.push({ licence_no: { [Op.in]: regNos } })
      if (primaryEmails.length > 0) orConditions.push({ email: { [Op.in]: primaryEmails } })

      const matchedUsers = await OldUser.findAll({
        where: { [Op.or]: orConditions },
        attributes: ['licence_no', 'email', 'profile_picture', 'user_picture'],
      })

      // Two lookup maps, resolved via the same /old/users/uploads/:filename
      // route serveProfilePicture exposes (handles uploads/users/ and
      // legacy bare-filename paths alike).
      ...
      matchedUsers.forEach((u: any) => {
        ...
        if (!pic) return
        const url = `https://data.erb.go.ug/old/users/uploads/${path.basename(pic)}`
        if (u.licence_no) pictureByRegNo.set(u.licence_no.trim(), url)
        if (u.email)      pictureByEmail.set(u.email.trim().toLowerCase(), url)
      })

      plainRecords.forEach(r => {
        const regNo = r.reg_no?.trim()
        const email = r.email_address?.split(';')[0]?.trim().toLowerCase()
        r.profile_picture_url =
          (regNo && pictureByRegNo.get(regNo)) ||
          (email && pictureByEmail.get(email)) ||
          null
      })
    } else {
